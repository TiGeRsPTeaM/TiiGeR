...
local lang = redis:get(hash) 
  ...
  elseif lang then
	...
	keyboard.inline_keyboard = {
		...
		{
			{text = '>♥️ درباره ما ', callback_data = '/tiiger:'..GP_id}
		},
		{
...
local settings = data[tostring(GP_id)]["settings"]
 ...
 text = '_Welcome To_ *More Settings*'
	...
	keyboard.inline_keyboard = {
		...
		{
			{text = "➕♥️", callback_data='/floodup:'..GP_id}, 
			{text = tostring(settings.num_msg_max), callback_data="TiiGeRTeaM"},
			{text = "➖♣️", callback_data='/flooddown:'..GP_id}
		},
		{
			{text = '>🔸 Character Sensitivity ', callback_data = 'TiiGeRTeaM'}
		},
		{
			{text = "➕♥️", callback_data='/charup:'..GP_id}, 
			{text = tostring(settings.set_char), callback_data="TiiGeRTeaM"},
			{text = "➖♣️", callback_data='/chardown:'..GP_id}
		},
		{
			{text = '>♦️ Flood Check Time ', callback_data = 'TiiGeRTeaM'}
		},
		{
			{text = "➕♥️", callback_data='/floodtimeup:'..GP_id}, 
			{text = tostring(settings.time_check), callback_data="TiiGeRTeaM"},
			{text = "➖♣️", callback_data='/floodtimedown:'..GP_id}
		},
		{
 ...
 text = '_به تنظیمات بیشتر خوش آمدید_'
	...
	keyboard.inline_keyboard = {
		...
		{
			{text = '>♦️ زمان بررسی پیام های مکرر ', callback_data = 'TiiGeRTeaM'}
		},
		{
			{text = "➕♥️", callback_data='/floodtimeup:'..GP_id}, 
			{text = tostring(settings.time_check), callback_data="TiiGeRTeaM"},
...
local lang = redis:get(hash) 
   ...
   elseif lang then
	...
	local text = ' 🇮🇷_لطفا زبان مورد نظر خود را انتخاب کنید_ 🇦🇺  ARASHWM\n\n'
	keyboard = {} 
	keyboard.inline_keyboard = {
		{
...
if matches[1] == '/link' then
   ...
   elseif lang then
		...
		if not linkgp then
   if not lang then
			text = "_♻First set a_ *link* _for group with using_ /setlink"
    elseif lang then
			text = "_♻ابتدا با دستور_ `تنظیم لینک` _لینک جدیدی برای گروه تعیین کنید_"
  end
		else
   if not lang then
